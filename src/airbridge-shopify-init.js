(function () {
  ...
  // ⚡ 전역에 초기화 함수 노출
  window.AirbridgeShopifyInit = function (analytics, config) {
    console.log("[Airbridge] Initializing v" + VERSION);
    console.log("[Airbridge] Config:", config);

    // analytics 접근 가능 확인
    if (!analytics || typeof analytics.subscribe !== "function") {
      console.error("[Airbridge] Invalid analytics object");
      return;
    }
      ...
      try {
        ...
        const payload = {
          ...
          semanticAttributes: {
            ...
            currency:
              cart?.totalPrice?.currencyCode || config.currency || "KRW",
          },
        };
